        ...
        <div class="glass border border-white/20 rounded-[2rem] px-8 py-4 shadow-2xl flex justify-between items-center">
            ...
            <div class="flex items-center gap-3">
                ...
                @if($business->whatsapp_number)
                    <a href="{{ route('business.track', ['businessId' => $business->id, 'type' => 'whatsapp_click', 'target' => 'https://wa.me/' . $business->whatsapp_number]) }}" class="w-10 h-10 bg-[#25d366] text-white rounded-full flex items-center justify-center hover:scale-110 transition-transform">💬</a>
                @endif
            </div>
        </div>
    </nav>
                ...
                @forelse($business->products as $product)
                    <div class="product-card group bg-white rounded-[3rem] p-6 shadow-xl border border-gray-50 transition-all hover:shadow-2xl hover:-translate-y-2" data-category="{{ $product->category->slug }}">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <a href="{{ route('business.track', ['businessId' => $business->id, 'type' => 'whatsapp_click', 'model_id' => $product->id, 'target' => 'https://wa.me/' . $business->whatsapp_number . '?text=' . urlencode('Hi, I am interested in ' . $product->name . ' from ' . $business->business_name . '.')]) }}" class="w-14 h-14 bg-[#25d366] text-white rounded-2xl flex items-center justify-center text-2xl shadow-lg shadow-[#25d366]/20 hover:scale-110 active:scale-95 transition-all">
                                💬
                            </a>
                        </div>
                    </div>
            ...
            <div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="flex items-center gap-6 group">
                        ...
                        <a href="{{ route('business.track', ['businessId' => $business->id, 'type' => 'whatsapp_click', 'target' => 'https://wa.me/' . $business->whatsapp_number]) }}" class="text-xl font-black">WhatsApp Chat</a>
                    </div>
                    @endif
                </div>
            </div>
                ...
                @else
                    <div class="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-[4rem] p-16 h-full text-white flex flex-col justify-center">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            @endif
                            @if($business->whatsapp_number)
                                <a href="{{ route('business.track', ['businessId' => $business->id, 'type' => 'whatsapp_click', 'target' => 'https://wa.me/' . $business->whatsapp_number]) }}" class="bg-white/20 backdrop-blur-md p-6 rounded-3xl text-center font-black hover:bg-white/30 transition-all">WhatsApp</a>
                            @endif
                        </div>
                    </div>
                @endif

...
<body class="bg-[#0f172a] text-slate-200 font-sans antialiased selection:bg-indigo-900 selection:text-white">
    ...
    <nav class="fixed w-full z-50 transition-all duration-300 bg-gradient-to-b from-black/80 to-transparent backdrop-blur-[2px] border-b border-white/5" id="navbar">
        <div class="max-w-7xl mx-auto px-6 py-6 flex justify-between items-center">
            <a href="#" class="flex items-center gap-2 md:gap-3 text-xl md:text-2xl font-serif italic font-bold text-white tracking-tighter hover:text-indigo-300 transition-colors overflow-hidden">
                @if($business->logo)
        ...
        <div id="mobile-menu" class="hidden absolute top-full left-0 w-full bg-[#1e293b] border-b border-slate-700 p-6 flex flex-col gap-6 text-center shadow-2xl">
            <a href="#vision" class="text-sm font-bold uppercase tracking-widest text-slate-400 hover:text-white" onclick="document.getElementById('mobile-menu').classList.add('hidden')">Our Vision</a>
            <a href="#services" class="text-sm font-bold uppercase tracking-widest text-slate-400 hover:text-white" onclick="document.getElementById('mobile-menu').classList.add('hidden')">Services</a>
            <a href="#contact" class="text-sm font-bold uppercase tracking-widest text-slate-400 hover:text-white" onclick="document.getElementById('mobile-menu').classList.add('hidden')">Contact</a>
            @if($business->whatsapp_number)
            <a href="https://wa.me/{{ $business->whatsapp_number }}" class="bg-white text-slate-900 px-6 py-3 rounded-full font-bold uppercase tracking-widest text-xs">Book Consultation</a>
            @endif
        </div>
    </nav>
    ...
    <header class="relative min-h-[500px] md:h-screen md:min-h-[600px] overflow-hidden flex items-center justify-center py-20">
        ...
        <div class="relative z-10 text-center px-6 max-w-4xl mx-auto fade-in">
            <div class="flex items-center justify-center gap-3 md:gap-4 mb-4 md:mb-6 opacity-80">
                ...
                <span class="text-slate-200 text-[8px] md:text-xs font-black uppercase tracking-[0.2em] md:tracking-[0.4em] whitespace-nowrap">Est. {{ $business->established_year ?? ($business->created_at ? $business->created_at->format('Y') : date('Y')) }}</span>
                <div class="w-8 md:w-16 h-px bg-white/60"></div>
            </div>
            ...
            <p class="text-slate-300 text-[10px] md:text-base font-medium tracking-[0.1em] md:tracking-[0.2em] mb-8 md:mb-12 uppercase max-w-2xl mx-auto leading-loose">
            ...
            <div class="flex flex-col md:flex-row gap-4 md:gap-6 justify-center items-center">
                @if($business->whatsapp_number)
                <a href="https://wa.me/{{ $business->whatsapp_number }}?text={{ urlencode('I would like to schedule a consultation.') }}" class="w-full md:w-auto bg-white text-black px-8 py-4 rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all hover:scale-105 shadow-[0_0_30px_rgba(255,255,255,0.1)]">
                    Schedule Consultation
                </a>
                @endif
                <a href="#services" class="group flex items-center gap-3 text-white text-[10px] font-black uppercase tracking-widest hover:text-indigo-300 transition-colors">

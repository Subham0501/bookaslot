...
<head>
    ...
    <title>{{ $business->business_name }} - Dining & Hospitality</title>
...
<body class="bg-[#0c0a09] text-stone-200 font-sans antialiased selection:bg-stone-700 selection:text-white">
    ...
    <nav class="fixed w-full z-50 transition-all duration-300 bg-gradient-to-b from-black/80 to-transparent backdrop-blur-[2px] border-b border-white/5" id="navbar">
        <div class="max-w-7xl mx-auto px-6 py-6 flex justify-between items-center">
            <a href="#" class="flex items-center gap-2 md:gap-3 text-xl md:text-2xl font-serif italic font-bold text-white tracking-tighter hover:text-stone-300 transition-colors overflow-hidden">
                @if($business->logo)
            ...
            <div class="hidden md:flex items-center gap-10 text-xs font-bold uppercase tracking-[0.2em] text-stone-400">
                <a href="#about" class="hover:text-white transition-colors">The Story</a>
                ...
                <a href="https://wa.me/{{ $business->whatsapp_number }}" class="bg-white text-stone-900 px-6 py-3 rounded-full hover:bg-stone-200 transition-transform hover:scale-105">Book Now</a>
                @endif
            </div>
        ...
        <!-- Mobile Menu -->
        <div id="mobile-menu" class="hidden absolute top-full left-0 w-full bg-[#1c1917] border-b border-stone-800 p-6 flex flex-col gap-6 text-center shadow-2xl">
            <a href="#about" class="text-sm font-bold uppercase tracking-widest text-stone-400 hover:text-white" onclick="document.getElementById('mobile-menu').classList.add('hidden')">The Story</a>
            <a href="#menu" class="text-sm font-bold uppercase tracking-widest text-stone-400 hover:text-white" onclick="document.getElementById('mobile-menu').classList.add('hidden')">Menu</a>
            <a href="#location" class="text-sm font-bold uppercase tracking-widest text-stone-400 hover:text-white" onclick="document.getElementById('mobile-menu').classList.add('hidden')">Location</a>
            @if($business->whatsapp_number)
            <a href="https://wa.me/{{ $business->whatsapp_number }}" class="bg-white text-black px-6 py-3 rounded-full font-bold uppercase tracking-widest text-xs">Book Now</a>
            @endif
        </div>
    </nav>
    ...
    <header class="relative min-h-[500px] md:h-screen md:min-h-[600px] overflow-hidden flex items-center justify-center py-20">
        ...
        <div class="absolute inset-0 z-0">
            ...
            @else
                <img src="https://images.unsplash.com/photo-1559339352-11d035aa65de?q=80&w=1974&auto=format&fit=crop" class="w-full h-full object-cover opacity-60">
            @endif
            <div class="absolute inset-0 bg-gradient-to-t from-[#0c0a09] via-[#0c0a09]/40 to-black/30"></div>
        </div>
        ...
        <div class="relative z-10 text-center px-6 max-w-4xl mx-auto fade-in">
            <div class="flex items-center justify-center gap-3 md:gap-4 mb-4 md:mb-6 opacity-80">
                ...
                <span class="text-stone-200 text-[8px] md:text-xs font-black uppercase tracking-[0.2em] md:tracking-[0.4em] whitespace-nowrap">Est. {{ $business->established_year ?? ($business->created_at ? $business->created_at->format('Y') : date('Y')) }}</span>
                <div class="w-8 md:w-16 h-px bg-white/60"></div>
            </div>
            ...
            <p class="text-stone-300 text-[10px] md:text-base font-medium tracking-[0.1em] md:tracking-[0.2em] mb-8 md:mb-12 uppercase max-w-2xl mx-auto leading-loose">
                {{ $business->category ?? 'Fine Dining & Hospitality' }}
            </p>
            
            <div class="flex flex-col md:flex-row gap-4 md:gap-6 justify-center items-center">
                @if($business->whatsapp_number)
                <a href="https://wa.me/{{ $business->whatsapp_number }}?text={{ urlencode('I would like to make a reservation.') }}" class="w-full md:w-auto bg-white text-black px-8 py-4 rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-stone-200 transition-all hover:scale-105 shadow-[0_0_30px_rgba(255,255,255,0.1)]">
                    Reserve a Table
                </a>
                @endif
                <a href="#menu" class="group flex items-center gap-3 text-white text-[10px] font-black uppercase tracking-widest hover:text-stone-300 transition-colors">
                    Explore Menu <span class="group-hover:translate-y-1 transition-transform text-lg">↓</span>
                </a>
            </div>
        </div>
    </header>
    ...
    <!-- About Section -->
    <section id="about" class="py-24 md:py-32 bg-[#0c0a09] relative overflow-hidden">
        <!-- Abstract Decoration -->
        <div class="absolute top-0 right-0 w-[500px] h-[500px] bg-stone-900/30 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none"></div>
        ...
        <div class="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-24 items-center">
            <div class="relative">
                <div class="aspect-[4/5] rounded-none overflow-hidden relative">
                     <!-- Try to find a nice 'about' image, fallback to first product or banner, else placeholder -->
                     @php
                ...
                <!-- Since aesthetic -->
                <div class="absolute -bottom-10 -right-10 w-40 h-40 border border-white/20 rounded-full flex items-center justify-center text-stone-500 animate-[spin_20s_linear_infinite]">
            ...
            <div class="md:text-left text-center">
                <span class="text-[#d4af37] text-[10px] font-black uppercase tracking-[0.4em] mb-4 block">The Experience</span>
                <h2 class="text-4xl md:text-6xl font-serif italic text-white mb-8 leading-tight">
                    Where flavor meets <br><span class="text-stone-600">artistry.</span>
                </h2>
                <div class="w-20 h-px bg-stone-800 mb-8 md:mx-0 mx-auto"></div>
                <p class="text-stone-400 text-sm md:text-base leading-relaxed font-light mb-10">
                    {{ $business->description ?? 'We believe that dining is not just about food; it is about the experience. From the ambiance to the plating, every detail is curated to transport you to a world of culinary delight.' }}
                </p>
                <div class="flex flex-col md:flex-row gap-8 text-xs font-bold uppercase tracking-widest text-[#d4af37]">
                    <div class="flex items-center gap-3">
                        <span class="text-2xl">✦</span> Premium Quality
                    </div>
                    <div class="flex items-center gap-3">
                        <span class="text-2xl">✦</span> Fresh Ingredients
                    </div>
                </div>
            </div>
        </div>
    ...
    <!-- Menu Section -->
    <section id="menu" class="py-24 bg-[#141211] relative border-t border-white/5">
        <div class="max-w-7xl mx-auto px-6">
            <div class="text-center mb-16">
                <span class="text-stone-600 text-[10px] font-black uppercase tracking-[0.4em] mb-4 block">Culinary Highlights</span>
                <h2 class="text-4xl md:text-5xl font-serif italic text-white">Our Signature Menu</h2>
            </div>
            
            <!-- Categories -->
             @if($business->categories->count() > 0)
            <div class="flex flex-wrap justify-center gap-4 mb-16">
                <button class="menu-tab active px-8 py-3 bg-white text-black rounded-full text-[10px] font-black uppercase tracking-widest transition-all hover:bg-stone-200" data-category="all">All</button>
                @foreach($business->categories as $category)
                <button class="menu-tab px-8 py-3 bg-transparent border border-stone-800 text-stone-400 rounded-full text-[10px] font-black uppercase tracking-widest transition-all hover:border-stone-600 hover:text-white" data-category="{{ $category->slug }}">
                    {{ $category->name }}
                </button>
                @endforeach
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div class="menu-item group cursor-pointer" data-category="{{ $product->category->slug ?? 'all' }}"
                     onclick="window.location.href='https://wa.me/{{ $business->whatsapp_number }}?text={{ urlencode('I am interested in ordering: ' . $product->name) }}'">
                    
                    <div class="relative h-72 overflow-hidden mb-6 filter sepia-[.2] group-hover:sepia-0 transition-all duration-700">
                        @if($product->image)
                        ...
                        @else
                            <div class="w-full h-full bg-stone-900 flex items-center justify-center text-4xl text-stone-800">🍽️</div>
                        @endif
                    ...
                    <div class="flex justify-between items-start mb-2">
                        <h3 class="text-2xl font-serif italic text-stone-200 group-hover:text-[#d4af37] transition-colors">{{ $product->name }}</h3>
                        @if($product->price)
                        <span class="text-[#d4af37] font-bold text-sm">Rs {{ number_format($product->price) }}</span>
                        @endif
                    </div>
                    <p class="text-stone-500 text-xs leading-relaxed line-clamp-2 max-w-[90%]">{{ $product->description }}</p>
                </div>
                 @empty
                <div class="col-span-full py-20 text-center text-stone-600 text-xs uppercase tracking-widest">
                    Updating our menu...
                </div>
                @endforelse
            </div>
            ...
            <div class="mt-20 text-center">
                <a href="#" class="inline-block border-b border-[#d4af37] text-[#d4af37] pb-1 text-xs font-bold uppercase tracking-widest hover:text-white hover:border-white transition-colors">View Complete Menu (PDF)</a>
            </div>
        </div>
    </section>

    <!-- Reviews / Stats Section -->
    <section class="py-20 bg-[#0c0a09] border-y border-stone-900">
        <div class="max-w-4xl mx-auto px-6 text-center">
            <span class="text-stone-600 text-[10px] font-black uppercase tracking-[0.4em] mb-8 block">Guest Reviews</span>
            <div class="text-3xl md:text-5xl font-serif italic text-stone-300 leading-tight mb-8">
                "An unforgettable evening. The service was impeccable, and the atmosphere was simply magical."
            </div>
            <div class="flex justify-center gap-1 text-[#d4af37] mb-4">
                ★★★★★
            </div>
            <p class="text-stone-500 text-xs font-bold uppercase tracking-widest">Priya Sharma — Google Reviews</p>
        </div>
    </section>
    ...
    <footer class="bg-[#0c0a09] py-12 border-t border-white/5 text-center">
        ...
        <div class="flex justify-center gap-8 mb-8 text-[10px] font-bold uppercase tracking-widest text-stone-500">
            ...
            <a href="#menu" class="hover:text-white transition-colors">Menu</a>
            <a href="#location" class="hover:text-white transition-colors">Contact</a>
        </div>
        ...
        tabs.forEach(tab => {
            tab.addEventListener('click', () => {
                tabs.forEach(t => {
                    ...
                    t.classList.add('bg-transparent', 'text-stone-400', 'border', 'border-stone-800');
                });
                tab.classList.remove('bg-transparent', 'text-stone-400', 'border', 'border-stone-800');

...
<div class="space-y-12">
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-4xl font-black text-gray-900 dark:text-white mb-2">Offers & Banners 🖼️</h1>
            <p class="text-gray-500 dark:text-[#94a3b8]">Keep your card fresh with active promotions.</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="col-span-full py-20 text-center">
            ...
            <h2 class="text-2xl font-black text-gray-300">No active offers.</h2>
            <p class="text-gray-400 font-bold mt-2">Banners appear at the top of your digital card.</p>
        </div>
        @endforelse
    </div>
</div>
...
<div id="addBannerModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen p-4">
        ...
        <div class="relative bg-white dark:bg-[#1e293b] rounded-[3rem] shadow-2xl w-full max-w-xl p-10">
            <h2 class="text-3xl font-black mb-8 text-gray-900 dark:text-white">New Banner</h2>
            
            <form action="{{ route('dashboard.banners.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                <div class="space-y-2">
                    <label class="text-xs font-black uppercase text-gray-400 tracking-widest">Offer Title</label>
                    <input type="text" name="title" class="w-full bg-gray-50 dark:bg-[#0f172a] border-none rounded-2xl p-4 focus:ring-2 focus:ring-[#ff6b6b] font-bold text-gray-900 dark:text-white" placeholder="e.g., Dashain Festival Offer">
                </div>
                ...
                <div class="space-y-2">
                    <label class="text-xs font-black uppercase text-gray-400 tracking-widest">Description</label>
                    <textarea name="description" rows="3" class="w-full bg-gray-50 dark:bg-[#0f172a] border-none rounded-2xl p-4 focus:ring-2 focus:ring-[#ff6b6b] font-bold text-gray-900 dark:text-white" placeholder="Details about this offer..."></textarea>
                </div>

                <button type="submit" class="w-full py-5 bg-[#ff6b6b] text-white rounded-2xl font-black text-xl shadow-xl shadow-[#ff6b6b]/20">Upload Banner</button>
            </form>
        </div>
    </div>
</div>
